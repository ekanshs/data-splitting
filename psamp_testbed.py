...
tr_model = NMF(n_components=K, init='random', random_state=0, max_iter=1000)
...
zi_ts_graph = zero_index_sparse_graph(zi_ts_graph, axis=0, convert=ts_I_zero_indexer)


topk = torch.topk(torch.tensor(predictions), n_ts_I)
# 3. predict graph_test: evaluate
score = nDCG(np.r_[0:ts_U.shape[0]], topk[1], zi_ts_graph[0])
